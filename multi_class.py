...
class one_vs_one:
    ...
    Attribute
    self.class_num: クラス数
    self.dim: 入力次元(今回は28*28)
    self.combinations: 組み合わせるクラスのリスト(1vs2, 1vs3, ..., N-1vsN)
    self.model_list: 2値分類器モデルのリスト(combinations分用意)
    ...
    def train(self, x):
        ...
        # i vs j (i=combi[0], j=combi[1])データセットを作成
...
class one_vs_other:
    ...
    self.class_num: クラス数
    self.dim: 入力次元(今回は28*28)
    self.model_list: 2値分類器モデルのリスト(クラス数)
    ...
    def train(self, x):
        ...
        # i vs other(i=label, other=label以外)データセットを作成
